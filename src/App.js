...
export default class App extends PureComponent {
  ...
  render() {
    ...
    let selectedEmail = EMAILS.find((email) => {
      return email.id === this.state.selectedEmailId
    })

    let emailViewComponent;

    if (selectedEmail) {
      emailViewComponent = (<EmailView email={selectedEmail}/>)
    }
    ...
    return (
      <main className="app">
        <EmailList emails={EMAILS} onItemSelect={this._handleItemSelect.bind(this)} />
        {emailViewComponent} 
        <EmailForm />
      </main>
    );
